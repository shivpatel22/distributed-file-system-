...
int main() {
    ...
    log_message("S4 started, listening on port 9083");
    ...
    while (1) {
        ...
        if (strcmp(command, "uploadf") == 0) {
            ...
            snprintf(filepath, MAX_PATH, "%s/%s", S4_DIR, arg2);
            ...
            char filepath[MAX_PATH];
            snprintf(filepath, MAX_PATH, "%s/%s", S4_DIR, arg1);
            ...
            tar_files("zip", "zip.tar");
            send_file(client_socket, "zip.tar");
            remove("zip.tar");
...
void tar_files(const char *filetype, const char *output_tar) {
    ...
    snprintf(cmd, BUFFER_SIZE, "find %s -name '*.%s' | xargs tar -cvf %s", S4_DIR, filetype, output_tar);
    system(cmd);
}
