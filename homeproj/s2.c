...
int main() {
    ...
    log_message("S2 started, listening on port 9081");
    ...
    while (1) {
        ...
        if (strcmp(command, "uploadf") == 0) {
            ...
            snprintf(filepath, MAX_PATH, "%s/%s", S2_DIR, arg2);
            ...
            char filepath[MAX_PATH];
            snprintf(filepath, MAX_PATH, "%s/%s", S2_DIR, arg1);
            ...
            tar_files("pdf", "pdf.tar");
            send_file(client_socket, "pdf.tar");
            remove("pdf.tar");
...
void tar_files(const char *filetype, const char *output_tar) {
    ...
    snprintf(cmd, BUFFER_SIZE, "find %s -name '*.%s' | xargs tar -cvf %s", S2_DIR, filetype, output_tar);
    system(cmd);
}
